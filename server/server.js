...
//save apiDocID to type for userID (RECIEVES and SETS ARRAY by overiding prev values)
//options: recents, saved
app.post("/user/:userId/saveArray/:type", (req, res) => {
  console.log("array overirde REQ RECEIVED!");
  const { userId, type } = req.params;
  const { docIds } = req.body;

  //sets prev array to new array
  db.collection("Users")
    .updateOne({ _id: new ObjectId(userId) }, { $set: { [type]: docIds } })
    .then((results) => {
      console.log("results");
      console.log(results);
      res.send(results);
...
//get API Docs for given array of ids
app.post("/read/ids/:limit/", async (req, res) => {
  const docIds = req.body;
  const { limit} = req.params;
  //limit amount of docs
  const objIds = docIds.map((docId, index) => {
    if (index >= limit) return;
    return new ObjectId(docId.trim());
  });
